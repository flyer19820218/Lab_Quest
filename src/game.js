(function () {
  ...
  function chargePoint(zone,slot,angle,positive=false){
    ...
    if(zone==='top'){
      // All six free-electron sites start on the upper metal disc. The fixed
      // positive sites keep their original spread across the upper conductor.
      const p=(positive?[[-.23,1.397],[-.08,1.397],[.08,1.397],[.23,1.397],[0,1.04],[0,.82]]:[[-.29,1.397],[-.18,1.397],[-.06,1.397],[.06,1.397],[.18,1.397],[.29,1.397]])[slot];
      return scope.localToWorld(V(p[0]+off,p[1],.07));
    }
  ...
  function animateCharges(now,dt){
    ...
    const pivot=scope.localToWorld(V(0,.59,.07)),disc=scope.localToWorld(V(0,1.345,.07)),stem=scope.localToWorld(V(0,.96,.07));
    negMarks.forEach((g,i)=>{const p=assignments[i];if(!p)return;const t=Math.min(1,(now-p.start)/950),end=chargePoint(p.zone,p.slot,a);g.visible=p.zone!=='earth'||(p.moving&&t<1);
      if(p.moving&&t<1){let route=[p.from];
        ...
        else route.push(disc,stem);
        // Match the source lesson: upper metal disc -> metal stem -> leaves.
        // Both induction stages begin at the disc, with no upward detour.
