...
March 走路版 demo（2026-09-25）：自由探索使用老師提供的 March 3D 走路模型。進入實驗桌後暫時隱藏人物，保留 3D 實驗桌與驗電器配置，由玩家直接連續調整帶電棒、接地和觀察電荷。
...
實驗桌、人物、器材是 WebGL 立體物件。`assets/characters/catalog.json` 收錄老師提供的 `march-walk.glb` 和 `xiaozhen-walk.glb`，對應原檔 `march走路.glb` 和 `曉臻老師走路.glb`，檔案位元組未修改。現在由 March 擔任自由探索玩家；曉臻老師先存入素材庫，尚未放進場景。兩個模型均包含 `Walk_Female` 走路片段。March 停下來時使用從完整步態產生的靜止站姿；模型載入失敗時會退回程序角色。進入實驗操作後不使用人物手臂動畫。舊的免費人物模型仍保留作為歷史素材，但遊戲不再載入。
...
這是最初階 3D 遊戲原型。已測 Chrome 桌面與橫式手機觸控模擬；March 版的真實 iPhone／iPad Safari 尚未覆測。March 原始 GLB 約 28.5 MB，手機首次下載需要一些時間。另有兩處待老師明確核准後才能修正：藍色電子動畫抵達鋁箔的視覺終點，以及第五題驗證文字與原教材逐字一致。未實作金屬球關卡、服裝切換、其他房間或手把操作。
